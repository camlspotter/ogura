...
from extract_training_text import ROOT, LINE, TOKEN, strip_han_ivs, digest, emit, split_article

OUT=ROOT/'data/training_text_direct_100'
REPORT=ROOT/'results/training_text_direct_100'
INDEX=ROOT/'data/wikipedia_source_index'
...
def main():
    ...
    parser.add_argument('--output',type=Path,default=ROOT/'data/training_text_direct_100')
    parser.add_argument('--report-dir',type=Path,default=ROOT/'results/training_text_direct_100')
    ...
    targets=list(map(json.loads,(ROOT/'results/charset_refined/targets.jsonl').open()))
    chars=[r['character'] for r in targets];vocab=set(chars);ids={c:i for i,c in enumerate(chars)}
    source=json.loads((ROOT/'results/source.json').read_text());seed=20260915;goal=args.goal
        ...
        for step,c in enumerate(order):
            ...
            for n in inv[ids[c]]:
                ...
                for line in LINE.finditer(a['text']):
                    if c not in line.group():continue
                    ...
                    for i,t in enumerate(ts):
                        ...
                        for length in range(min(25,hi-lo),args.min_length-1,-1):
                            for start in range(max(lo,i-length+1),min(i,hi-length)+1):
                                ...
                                if key in hashes or key in candidates:continue
    ...
    import shutil
    shutil.copyfile(ROOT/'results/charset_refined/targets.jsonl',OUT/'targets.jsonl')
    ...
    metadata={'seed':seed,'target_samples_per_entry':goal,'target_entries':len(chars),'target_sha256':__import__('hashlib').sha256((OUT/'targets.jsonl').read_bytes()).hexdigest(),
        ...
        'strip_han_ivs':True,'allow_source_overlap':True,'max_samples_per_anchor_per_article':None,'max_tokens':25,'min_tokens':args.min_length,
        ...
        'limitations':'Greedy, not a minimum. May reuse source occurrences in different windows. No synthesized text. Validation/test articles excluded.'}
    statuses=Counter()
