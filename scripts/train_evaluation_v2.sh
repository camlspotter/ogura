...
if [[ "$resume" == false ]]; then
  set -- --init-from runs/noto48-residual64-quotes-homoglyphs/latest.pt "$@"
fi
exec uv run --frozen --extra train python -m ogura.training.train \
  --device cuda \
  --run-dir runs/noto48-residual64-evaluation-v2 \
  ...
  --validation-text datasets/japanese_english_quotes/validation/validation.txt \
  --validation-augmented --selection-metric mean-set-cer \
  ...
  --monitor-validation datasets/evaluation_v2/validation/quotes/validation.txt \
  --monitor-validation datasets/evaluation_v2/validation/homoglyphs/validation.txt \
