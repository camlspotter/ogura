> 以下のコマンドはレポジトリ直下で `cd textrec` してから実行してください。
...
```sh
uv run --frozen --extra train python -m ogura.textrec.training.train \
...
```sh
uv run --frozen python -m ogura.textrec.build_validation
```
...
```sh
uv run --frozen python -m ogura.textrec.download_training_fonts
```
...
```sh
uv run --frozen --extra train python -m ogura.textrec.training.preview \
...
```sh
uv run --frozen --extra train python -m ogura.textrec.evaluate_lengths \
...
```sh
uv run --frozen --extra train python -m ogura.textrec.recognize \
...
```sh
uv run --frozen python -m ogura.textrec.download_training_fonts --include-rounded
...
```sh
uv run --frozen python -m ogura.textrec.download_training_fonts --include-rounded --include-western
uv run --frozen --extra train python -m ogura.textrec.training.preview \
...
```sh
uv run --frozen --extra train python -m ogura.textrec.diagnose_validation \
...
`ogura.textrec.evaluate_lengths` も同じ `--evaluation-aliases` オプションを受け付ける。
...
```bash
uv run --frozen python -m ogura.textrec.build_english
```
...
```bash
uv run --frozen python -m ogura.textrec.prepare_english_training
...
折り返された端末表示ではなく、`ogura.textrec.recognize --output` の元JSONLを入力する。

```bash
uv run --frozen --extra train python -m ogura.textrec.evaluate_real \
...
```bash
uv run --frozen python -m ogura.textrec.refine_charset --output charset/selected_quotes
```
...
```bash
uv run --frozen python -m ogura.textrec.build_quotes
...
```bash
uv run --frozen python -m ogura.textrec.prepare_quote_training
```
...
```sh
uv run --frozen --extra train python -m ogura.textrec.build_evaluation_suite
```
...
```sh
uv run --frozen --extra train python -m ogura.textrec.evaluate_suite \
...
単独評価の `ogura.textrec.evaluate_suite` も `--selection-metric mean-set-weighted-cer` に対応する。
...
```sh
uv run --frozen python -m ogura.textrec.collect_latin_sequences \
...
uv run --frozen python -m ogura.textrec.collect_latin_sequences \
  --output datasets/latin_sequences_wikipedia_lowercase
uv run --frozen python -m ogura.textrec.prepare_latin_training
...
```sh
uv run --frozen python -m ogura.textrec.build_latin_validation
uv run --frozen --extra train python -m ogura.textrec.evaluate_latin_sequences \
...
`ogura.textrec.evaluate_latin_sequences` で行う。CTC損失や学習データは変更しない。
