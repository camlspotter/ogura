...
import torch
from ogura.textrec.training.train import TrainConfig, train, sha256, selection_score, load_initial_weights
...
class SelectionTests(unittest.TestCase):
    ...
    def test_latest_warmstart_initial_best_and_resume(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch('ogura.textrec.training.train.evaluate',side_effect=main_eval), \
                 patch('ogura.textrec.evaluate_lengths.evaluate_sets',side_effect=monitors), redirect_stdout(io.StringIO()):
