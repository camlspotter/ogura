...
from ogura.textrec.text_common import digest
...
class SynthesisTest(unittest.TestCase):
    ...
    def test_repeatable_distinct_donor_build(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for name in ['a','b']:
                subprocess.run([sys.executable,'-m','ogura.textrec.synthesize_shortfalls','--base',str(base),'--donor-index',str(index),'--output',str(root/name),'--report-dir',str(root/(name+'report')),'--goal','2','--seed','7'],cwd=Path(__file__).resolve().parents[1],check=True,stdout=subprocess.DEVNULL)
