...
from ogura.textrec.training.render import replace_unsupported, render_sample, Sample, RenderParams, BatchRenderer, Vocabulary
...
class SpaceNormalizationTests(unittest.TestCase):
    def test_missing_existing_and_edge_spaces(self):
        with patch('ogura.textrec.training.render.font_characters', return_value=frozenset(map(ord, ' 日本文\u3000'))):
    ...
    def test_mixed_font_edge_spaces(self):
        from ogura.textrec.training.render import normalized_text
        with patch('ogura.textrec.training.render.font_characters', return_value=frozenset(map(ord, ' A日本\u3000'))):
