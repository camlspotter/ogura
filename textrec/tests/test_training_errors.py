...
import torch
from ogura.textrec.training.error_samples import error_rows
from ogura.textrec.training.render import Sample,RenderParams
...
class ErrorSamplesTests(unittest.TestCase):
    ...
    def test_checkpoint_offset_and_resume_truncates_uncommitted_tail(self):
        with tempfile.TemporaryDirectory() as tmp,redirect_stdout(io.StringIO()):
            ...
            with patch('ogura.textrec.training.train.decode',return_value=['日']):train(cfg)
            ...
            with patch('ogura.textrec.training.train.decode',return_value=['日']):train(replace(cfg,resume=True,max_steps=101))
