...
from ogura.textrec.text_common import ROOT, LINE, TOKEN, strip_han_ivs, digest, emit, split_article
from ogura.textrec.diversity import DiversityFilter, VERSION, SHINGLE_LENGTH
...
def main():
    ...
    source=json.loads((CORPUS_ROOT / 'wikipedia/source.json').read_text());seed=20260915;goal=args.goal
    ...
    if not (INDEX/'complete.pkl').exists():
        ...
        with (INDEX/'article_splits.jsonl').open('w') as sf:
            for file in source['files']:
                path=CORPUS_ROOT / 'wikipedia/20231101.ja'/Path(file['path']).name
