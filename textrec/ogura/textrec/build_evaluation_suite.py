...
from ogura.textrec.build_english import candidate as english_candidate, download, CONFIG, REVISION
from ogura.textrec.build_validation import sha256
from ogura.textrec.diversity import fingerprints
from ogura.textrec.prose_filter import is_prose
from ogura.textrec.text_common import ROOT, split_article, strip_han_ivs, digest
...
def build(args):
    ...
    english, _ = download(CORPUS_ROOT / 'wikipedia'/CONFIG)
    japanese = sorted((CORPUS_ROOT / 'wikipedia/20231101.ja').glob('*.parquet'))
    if not japanese:
