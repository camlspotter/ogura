...
from ogura.textrec.training.metrics import weighted_cer_label
...
def validation_sets(config, vocabulary, paths, identity, all_fonts=False):
    from ogura.textrec.training.train import EpochDataset, prepare_data, sha256
    result = []
    for path in paths:
        path = Path(path)
...
def load_validation_context(checkpoint, run_config=None, font_dir=None):
    ...
    from ogura.textrec.training.train import TrainConfig, sha256
