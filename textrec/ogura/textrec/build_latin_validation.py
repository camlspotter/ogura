...
from ogura.textrec.build_english import download, sha, CONFIG, REPO, REVISION, SHARD, SHA256
from ogura.textrec.collect_latin_sequences import PATTERNS, paragraphs, excerpts, training_article
...
def build(args):
    ...
    try:
        ...
        manifest=dict(split='validation',samples=len(rows),min_length=min(lengths),max_length=max(lengths),
                      ...
                      dependencies_sha256={str(p):sha(p) for p in [ROOT/'ogura/textrec/collect_latin_sequences.py',ROOT/'ogura/textrec/build_english.py',ROOT/'ogura/textrec/diversity.py',ROOT/'ogura/textrec/training/aliases.py']},
