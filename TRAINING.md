...
`ogura.recognize` に `--width-scale 1 --width-scale 1.5 --width-scale 2` を
指定すると、同一モデルで入力画像の横幅だけを変えて認識する。
高さは48pxのままで、文字形状も横長になる。これは再学習を伴わない診断実験であり、
改善を保証しない。JSONLに倍率・入力幅・CTC出力位置数を記録する。
`--save-inputs` を併用すると各倍率の入力画像も保存できる。
保存済みvalidation画像の比較では `--preprocessing none` を使う。
