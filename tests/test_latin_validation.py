...
from ogura.build_latin_validation import build, eligible
from ogura.collect_latin_sequences import training_article
from ogura.evaluate_latin_sequences import sequence_counts, rates
...
class SequenceMetricsTests(unittest.TestCase):
    ...
    def test_fixed_images_comparison_smoke(self):
        import sys
        import torch
        from ogura.evaluate_latin_sequences import main
        from ogura.training.train import EpochDataset,TrainConfig
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.object(sys,'argv',args),patch('ogura.evaluate_latin_sequences.load_validation_context',return_value=(state,vocab,config)),patch('ogura.evaluate_latin_sequences.validation_sets',return_value=[({'mode':m},dataset) for m in ('baseline','augmented')]):
                main()
...
class LatinValidationBuildTests(unittest.TestCase):
    def test_validation_only_provenance_and_repeatability(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch('ogura.build_latin_validation.download',return_value=(source,'fixture')):
