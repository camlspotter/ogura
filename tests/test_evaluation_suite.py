import unittest
from ogura.build_evaluation_suite import choose, candidates
...
class EvaluationSuiteTests(unittest.TestCase):
    ...
    def test_build_writes_reserved_test_and_preserves_old_quotes(self):
        ...
        from ogura.build_evaluation_suite import build
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch('ogura.build_evaluation_suite.ROOT',root),patch('ogura.build_evaluation_suite.download',return_value=(en,'url')),patch('ogura.build_evaluation_suite.split_article',side_effect=lambda aid,seed:'test' if int(aid)<2 else 'validation'),patch('ogura.build_evaluation_suite.candidates',side_effect=excerpts):
                build(args)
