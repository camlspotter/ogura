#!/usr/bin/env bash
# Run from any working directory; existing text-only pages and models are preserved.
set -euo pipefail
cd "$(dirname "${BASH_SOURCE[0]}")/.."
export PLAYWRIGHT_BROWSERS_PATH="${PLAYWRIGHT_BROWSERS_PATH:-$PWD/.cache/playwright}"
tables=outputs/synth-tables-2000-v1
mixed=outputs/synth-mixed-7000-v1
case "${1:-}" in
  generate|resume)
    resume_args=()
  ...
  combine)
    uv run --locked python -m ogura.textdet.combine_synthetic \
      --text outputs/synth-jdoc-5000-v1 \
  ...
  train)
    ...
    model_output=outputs/db-resnet34-synth7000-v1
    ...
    ;;
  *) echo 'Usage: bash scripts/synth_tables_2000.sh generate|resume|combine|train' >&2; exit 2 ;;
esac
