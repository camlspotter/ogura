...
def load_image(path, preprocessing='none'):
...
def main():
    ...
    parser.add_argument('--preprocessing', choices=('none', 'otsu'), default='none')
    parser.add_argument('--compare-preprocessing', action='store_true',
                        help='Recognize each image both unchanged and with Otsu binarization')
    ...
    modes = ('none', 'otsu') if args.compare_preprocessing else (args.preprocessing,)
    ...
    with torch.inference_mode():
        for index, path in enumerate(args.images, 1):
            for mode in modes:
                pixels, widths = load_image(path, mode)
                if args.save_inputs:
