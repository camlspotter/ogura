#!/usr/bin/env bash
# Run from the repository root. Reuse reviewed assets; no remote access.
set -euo pipefail
export PLAYWRIGHT_BROWSERS_PATH="${PLAYWRIGHT_BROWSERS_PATH:-$PWD/ogura/textdet/.cache/playwright}"
export UV_CACHE_DIR="${UV_CACHE_DIR:-$PWD/ogura/textdet/.cache/uv}"
action=${1:-generate}
count=${2:-12}
output=${3:-ogura/textdet/outputs/synth-image-pages-pilot-v2}
...
for kind in text tables; do
  ...
  uv run --locked python -m ogura.textdet.synth_jdoc ${extra[@]+"${extra[@]}"} \
    ...
    --image-assets ogura/textdet/outputs/image-assets-200-v1/images --image-position both --image-float-fraction .5 \
