...
evaluation=$root/validation-synth11000-v1
case "${1:-}" in
  generate|resume)
  ...
  evaluate)
    ...
    CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}" uv run --locked python \
      -m ogura.textdet.diagnose_postprocess --amp --all-pages --expected-pages 39 \
    ...
    ;;
  package)
    uv run --locked python -c 'import json,sys; from pathlib import Path; m=json.loads((Path(sys.argv[1])/"summary.json").read_text()); assert m["status"]=="complete" and m["pages"]==39 and set(m["checkpoints"])=={"synth9000","synth11000"}' "$evaluation"
    if [[ -e "$evaluation.tar.gz" ]]; then echo "Already exists: $evaluation.tar.gz" >&2; exit 1; fi
    tar --exclude='*.npy' -czf "$evaluation.tar.gz.tmp" -C "$root" validation-synth11000-v1
    mv "$evaluation.tar.gz.tmp" "$evaluation.tar.gz"
    ;;
  download-command)
    echo 'Run on your Mac:'
    echo 'scp dgx:~/ogura/ogura/textdet/outputs/validation-synth11000-v1.tar.gz ~/ogura/ogura/textdet/outputs/'
    echo 'tar -xzf ~/ogura/ogura/textdet/outputs/validation-synth11000-v1.tar.gz -C ~/ogura/ogura/textdet/outputs/'
    ;;
  all)
