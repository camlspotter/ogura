#!/usr/bin/env bash
set -euo pipefail
output=ogura/textdet/outputs/postprocess-diagnosis-v1
case "${1:-run}" in
  run)
  ...
  package)
    ...
    tar -czf "$output.tar.gz.tmp" -C ogura/textdet/outputs postprocess-diagnosis-v1
    mv "$output.tar.gz.tmp" "$output.tar.gz"
    ;;
  download-command)
    echo 'scp dgx:~/ogura/ogura/textdet/outputs/postprocess-diagnosis-v1.tar.gz ~/ogura/ogura/textdet/outputs/'
    echo 'tar -xzf ~/ogura/ogura/textdet/outputs/postprocess-diagnosis-v1.tar.gz -C ~/ogura/ogura/textdet/outputs/'
    ;;
