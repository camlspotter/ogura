...
from ogura.build_english import CONFIG, SHARD, SHA256, REVISION, REPO, download, prose, sha, LATIN
...
def build(args):
    ...
    try:
        ...
        report=dict(source=dict(repository=REPO,revision=REVISION,config=CONFIG,shard=SHARD,sha256=SHA256,url=url,
                    ...
                    implementation_sha256={str(p):sha(p) for p in [Path(__file__),Path(__file__).with_name('build_english.py'),Path(__file__).with_name('diversity.py'),ROOT/'ogura/training/aliases.py']},
